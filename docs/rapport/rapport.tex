...
Le frontend de \bcaas{} est développé avec Next.js 16 (App Router) et React 19. Il matérialise le plan de données vu de l'extérieur~:
\begin{description}
	\item[Site vitrine (data plane view)] présentation publique de l'\api, documentation interactive et assistant conversationnel.
	\item[Tableau de bord Admin (control plane)] interface sécurisée pour la gestion des tenants, businesses, acteurs, services, demandes et factures. \emph{Prévu dans une version future, seules les ébauches de pages sont présentes dans le code actuel.}
\end{description}
...
\subsection{Tableau de bord admin (à venir)}
Les ébauches de pages listées ci-dessous sont présentes dans le code, mais l'intégration fonctionnelle complète du tableau de bord est repoussée à une version ultérieure~:
\begin{itemize}
...
\begin{itemize}
	...
	\item un frontend Next.js 16 / React 19 (site vitrine, documentation, assistant IA) déployé sur Vercel/Render. Le tableau de bord d'administration est en cours de développement.
\end{itemize}
