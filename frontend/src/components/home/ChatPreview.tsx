...
export function ChatPreview() {
  return (
    <section className="py-24 px-6 border-t border-white/[0.06] bg-[#0A0A0F]">
      <div className="max-w-5xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <motion.div initial={{opacity:0,x:-20}} whileInView={{opacity:1,x:0}} viewport={{once:true}} transition={{duration:0.6}}>
            ...
            <h2 className="font-display text-[32px] font-bold tracking-tight text-white mb-4">
              Votre assistant<br/><span className="gradient-text">intelligent</span>
            </h2>
            <p className="text-[15px] leading-relaxed mb-6 text-gray-400">
          ...
          <motion.div initial={{opacity:0,x:20}} whileInView={{opacity:1,x:0}} viewport={{once:true}} transition={{duration:0.6,delay:0.1}}
            className="rounded-2xl border border-white/10 overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.4)]"
            style={{background:"#111118"}}>
            <div className="flex items-center gap-3 px-4 py-3 border-b border-white/[0.07]" style={{background:"#16161F"}}>
              <div className="w-8 h-8 rounded-lg bg-gradient-brand flex items-center justify-center text-white text-[13px] font-bold">B</div>
              <div>
                <div className="text-[13px] font-semibold text-white">BizCore AI</div>
            ...
            <div className="p-4 flex flex-col gap-3">
              {[
                ...
                { role:"bot",  text:<span>Envoyez un <code className="text-[10.5px] px-1.5 py-0.5 rounded font-mono bg-white/10">POST /api/service-requests</code> avec votre JWT et <code className="text-[10.5px] px-1.5 py-0.5 rounded font-mono bg-white/10">X-Tenant-Id</code>. Statut initial : <code className="text-[10.5px] px-1.5 py-0.5 rounded font-mono bg-white/10">PENDING</code>.</span> },
              ].map((msg,i)=>(
                <motion.div key={i} initial={{opacity:0,y:8}} whileInView={{opacity:1,y:0}} viewport={{once:true}}
                  ...
                  <div className={`px-3.5 py-2.5 rounded-2xl text-[12.5px] leading-relaxed ${
                    msg.role==="user" ? "bg-indigo-600 text-white rounded-br-sm" : "bg-white/5 border border-white/[0.07] text-gray-300 rounded-bl-sm"}`}>
                    {msg.text}
                  </div>
                </motion.div>
              ))}
            ...
            <div className="px-4 pb-4">
              <div className="flex gap-2 items-center px-3 py-2 rounded-xl border border-white/10 bg-white/5">
                <span className="flex-1 text-[12.5px] text-gray-600">Posez votre question…</span>
