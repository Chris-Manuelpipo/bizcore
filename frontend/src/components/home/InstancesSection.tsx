    ...
    <section className="py-24 px-6 bg-[#0A0A0F]">
      <div className="max-w-5xl mx-auto">
        <motion.div initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.6}} className="text-center mb-14">
          ...
          <h2 className="font-display text-[32px] font-bold tracking-tight text-white">Des métiers concrets,<br/><span className="gradient-text">une seule API</span></h2>
        </motion.div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {INSTANCES.map((inst,i) => (
            <motion.div key={i} initial={{opacity:0,scale:0.95}} whileInView={{opacity:1,scale:1}} viewport={{once:true}}
              ...
              className="p-5 rounded-2xl border border-white/[0.07] bg-[#111118] text-center cursor-default group">
              <div className="text-3xl mb-3 transition-transform duration-300 group-hover:scale-110 inline-block">{inst.emoji}</div>
              <div className="text-[13.5px] font-semibold mb-1 text-white">{inst.name}</div>
              <div className="text-[11px] mb-3 text-gray-500">{inst.group}</div>
        ...
        <motion.div initial={{opacity:0,y:16}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:0.4,duration:0.6}}
          className="mt-8 p-4 rounded-2xl border border-white/[0.06] bg-white/[0.02] text-center">
          <p className="text-[12px] text-gray-500">
            Toutes les instances consomment la même API — différenciées uniquement par leur{" "}
            <code className="text-[11.5px] px-1.5 py-0.5 rounded font-mono bg-white/5 text-indigo-400">X-Tenant-Id</code>
          </p>
        </motion.div>
      </div>
    </section>
