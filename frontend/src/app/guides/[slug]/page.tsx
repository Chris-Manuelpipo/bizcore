...
function CodeBlock({ body, lang, filename }: { body:string; lang:string; filename?:string }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden border border-white/10 mt-4">
      <div className="flex items-center justify-between px-4 py-2 border-b border-white/[0.07] bg-[#16161F]">
        <div className="flex items-center gap-2">
          <span className={cn("text-[11px] font-mono font-semibold", LANG_COLOR[lang] ?? "text-gray-400")}>{lang}</span>
          {filename && <span className="text-[11px] text-gray-600">— {filename}</span>}
        </div>
        <button onClick={copy} className="flex items-center gap-1.5 text-[11px] px-2 py-1 rounded-lg bg-white/5 text-gray-400 hover:text-white transition-colors">
          {copied ? <Check size={11} className="text-emerald-400" /> : <Copy size={11} />}
          {copied ? "Copié !" : "Copier"}
        </button>
      </div>
      <pre className="p-4 text-[12.5px] font-mono overflow-x-auto leading-relaxed bg-[#0A0A0F] text-gray-300 whitespace-pre-wrap">
        <code>{body}</code>
      </pre>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0F] pt-20">
      <div className="max-w-4xl mx-auto px-6 py-10">
        ...
        {/* Back */}
        <Link href="/guides" className="inline-flex items-center gap-1.5 text-[13px] text-gray-500 hover:text-white transition-colors mb-8">
        ...
        <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.5 }} className="mb-10">
          <div className="flex items-center gap-3 mb-3 flex-wrap">
            ...
            <span className="flex items-center gap-1 text-[12px] text-gray-500"><Clock size={11}/>{guide.duration}</span>
            <span className={cn("text-[10.5px] px-2.5 py-0.5 rounded-full border font-medium", DIFFICULTY_STYLE[guide.difficulty])}>
              {guide.difficulty}
            </span>
          </div>
          <h1 className="font-display text-[36px] font-extrabold tracking-tight text-white mb-3">{guide.title}</h1>
          <p className="text-[15px] text-gray-400 leading-relaxed max-w-2xl">{guide.description}</p>
          <div className="flex gap-1.5 flex-wrap mt-4">
            {guide.tags.map(t => (
              <span key={t} className="text-[11px] px-2 py-0.5 rounded bg-white/5 border border-white/[0.07] text-gray-500 font-mono">{t}</span>
            ))}
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[220px_1fr] gap-8">
          ...
          <div className="lg:sticky lg:top-20 lg:h-fit">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-600 mb-3">Étapes</p>
              ...
              {guide.steps.map((s, i) => (
                <button key={i} onClick={() => setActiveStep(i)}
                  ...
                  <span className={cn(
                    ...
                    activeStep === i ? "bg-indigo-500 text-white" : i < activeStep ? "bg-emerald-500/20 text-emerald-400" : "bg-white/10 text-gray-500"
                  )}>
                    {i < activeStep ? "✓" : i+1}
                  </span>
          ...
          <div>
            <motion.div key={activeStep} initial={{ opacity:0, x:12 }} animate={{ opacity:1, x:0 }} transition={{ duration:0.3 }}>
              <div className="flex items-center gap-3 mb-5">
                ...
                <h2 className="font-display text-[22px] font-bold text-white">{step.title}</h2>
              </div>

              <p className="text-[14px] text-gray-400 leading-relaxed">{step.content}</p>
              ...
              {step.tip && (
                <div className="mt-4 flex gap-3 p-4 rounded-xl border border-cyan-400/20 bg-cyan-400/5">
                  ...
                  <p className="text-[13px] text-gray-300 leading-relaxed">{step.tip}</p>
                </div>
              )}
              ...
              {step.warning && (
                <div className="mt-4 flex gap-3 p-4 rounded-xl border border-amber-400/20 bg-amber-400/5">
                  ...
                  <p className="text-[13px] text-gray-300 leading-relaxed">{step.warning}</p>
                </div>
              )}
            </motion.div>
            ...
            <div className="flex items-center justify-between mt-10 pt-6 border-t border-white/[0.07]">
              <button onClick={() => setActiveStep(p => p-1)} disabled={isFirst}
                className="flex items-center gap-2 px-4 py-2 rounded-xl border border-white/10 text-[13px] text-gray-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-all hover:bg-white/5">
